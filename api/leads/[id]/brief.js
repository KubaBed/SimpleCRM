...
export default async function handler(req, res) {
  ...
  const { data: lead, error: fetchErr } = await supabase
    .from('leads')
    .select('id, website, notes, first_name, last_name, company_name, industry, company_size, source, estimated_value, phone, email')
    .eq('id', id)
    .single()
  ...
  const newNotes = mergeNotes(lead.notes, briefMd)
